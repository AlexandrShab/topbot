...
print_r($table_name->table_name);
    ...
    function saveGarages($garsArr)
    {
        ...
        foreach($garsArr as $gar)
        {
            ...
            $query = "SELECT gar FROM garage WHERE player_id='$player_id' AND date='$date'";
            $data = $base->prepare($query);
            $data->execute(); 
            $savedGar = $data->fetch(PDO::FETCH_OBJ);
            //var_dump($savedGar);
            if($savedGar)
            {
                $query = "UPDATE garage SET gar='$garage' WHERE player_id='$player_id' AND date='$date';";
                $data = $base->prepare($query);
                $data->execute();
                $res = true;
            }
    ...
    function saveKm($kmsArr)
    {
        ...
        foreach($kmsArr as $chest)
        {
            ...
            $query = "SELECT km FROM chest WHERE player_id='$player_id' AND date='$date'";
            $data = $base->prepare($query);
            $data->execute(); 
            $savedKm = $data->fetch(PDO::FETCH_OBJ);
            //var_dump($savedKm);
            if($savedKm)
            {
                $query = "UPDATE chest SET km='$km' WHERE player_id='$player_id' AND date='$date';";
                $data = $base->prepare($query);
                $data->execute();
                $res = true;
            }
    ...
    public function updatePlayerName($player)
    {
        $id = $player['id'];
        $name = $player['name'];
        $db = new Connect;
